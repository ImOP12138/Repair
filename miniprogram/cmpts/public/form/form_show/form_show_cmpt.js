...
Component({
	 ...
	 */
	observers: {
		'forms': function (newForms) {
			if (newForms && newForms.length > 0) {
				if (this.data.isLoad) {
					this._updateFormValues(newForms);
				} else {
					this._init();
				}
			}
	...
	methods: {
		...
		_getOneValForm: function (mark, title, type) {
			...
			for (let k = 0; k < forms.length; k++) {
				if (forms[k].mark == mark) { // 优先匹配mark，不限制type
					ret = forms[k].val;
					break;
				}

				if (forms[k].title == title) { // 再则匹配名称，不限制type
					ret = forms[k].val;
					break;
				}
			...
			if (ret === null && this.data.isCacheMatch
				...
				if (caches && Array.isArray(caches)) {
					for (let k = 0; k < caches.length; k++) {
						if (caches[k].mark == mark) { // 优先匹配mark，不限制type
							ret = caches[k].val;
							break;
						}

						if (caches[k].title == title) { // 再则匹配名称，不限制type
							ret = caches[k].val;
							break;
						}
			...
			if (ret === null && this.data.isDefMatch) {
				...
				for (let k = 0; k < fields.length; k++) {
					if (fields[k].mark == mark) {
						// 优先使用 def 属性
						if (helper.isDefined(fields[k].def) && fields[k].def != null) {
							ret = fields[k].def;
							break;
						}
						// 再使用 val 属性（用于只读字段预设值）
						if (helper.isDefined(fields[k].val) && fields[k].val !== null && fields[k].val !== '') {
							ret = fields[k].val;
							break;
						}
					}
		...
		_updateFormValues: function (inputForms) {
			let forms = inputForms || this.data.forms;
			let fields = this.data.fields;
			let updated = false;

			for (let k = 0; k < fields.length; k++) {
				for (let j = 0; j < forms.length; j++) {
					if (fields[k].mark == forms[j].mark && forms[j].val !== undefined) {
						if (fields[k].val !== forms[j].val) {
							fields[k].val = forms[j].val;
							updated = true;
						}
						break;
					}
		...
		bindUseCommonAddress: function () {
			...
			this.setOneFormVal('address', this.data.commonAddress);
		}
	},
