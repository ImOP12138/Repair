...
Page({
	...
	bindUnbindWxTap: async function () {
		let memberId = this.data.id;
		if (!memberId) return;

		wx.showModal({
			title: '解绑微信号',
			content: '确定要解绑该工作人员的微信号吗？解绑后该人员将无法自动登录。',
			success: async (res) => {
				if (res.confirm) {
					try {
						let params = { memberId };
						await cloudHelper.callCloudSumbit('admin/member_unbind_wx', params);
						pageHelper.showSuccToast('解绑成功');
					} catch (err) {
						console.log(err);
					}
				}
