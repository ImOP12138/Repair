const WorkBiz = require('../../../../biz/work_biz.js');
const pageHelper = require('../../../../../../helper/page_helper.js');
const cloudHelper = require('../../../../../../helper/cloud_helper.js');

Page({

	/**
	 * 页面的初始数据
	 */
	data: {
		id: ''
	},

	/**
	 * 生命周期函数--监听页面加载
	 */
	onLoad: async function (options) {
	
	},

	/**
	 * 页面相关事件处理函数--监听用户下拉动作
	 */
	onPullDownRefresh: async function () {
		await this._loadDetail();
		wx.stopPullDownRefresh();
	},

	_loadDetail: async function () {
		try {
			let opts = {
				title: 'bar'
			}
			let res = await cloudHelper.callCloudData('work/home', {}, opts);
			this.setData({
				info: res
			});

		} catch (err) {
			console.log(err);
		}
	},

	/**
	 * 生命周期函数--监听页面初次渲染完成
	 */
	onReady: function () {

	},

	/**
	 * 生命周期函数--监听页面显示
	 */
	onShow: async function () {
		if (!WorkBiz.isWork(this)) {
			this.setData({
				isLoad:false
			});
			return;
		}
		
		// 用于修改图片后更新展示
		let work = WorkBiz.getWorkToken();
		this.setData({
			isLoad: true,
			work,
			id: work.id
		});

		await this._loadDetail();
	},

	/**
	 * 生命周期函数--监听页面隐藏
	 */
	onHide: function () {

	},

	/**
	 * 生命周期函数--监听页面卸载
	 */
	onUnload: function () {

	},

	url: function (e) {
		pageHelper.url(e, this);
	},



	bindExitTap: function (e) {
		wx.stopPullDownRefresh();
		let callback = function () {
			WorkBiz.clearWorkToken();
			wx.reLaunch({
				url: '../../../my/index/my_index',
			});
		}
		pageHelper.showConfirm('您确认退出?', callback);
	},

	bindWxTap: async function (e) {
		wx.showActionSheet({
			itemList: ['绑定微信', '解绑微信'],
			success: async (res) => {
				if (res.tapIndex === 0) {
					this.doBindWorkWx();
				} else if (res.tapIndex === 1) {
					this.doUnbindWorkWx();
				}
			}
		});
	},

	doBindWorkWx: async function () {
		wx.showModal({
			title: '绑定微信号',
			content: '绑定后，下次打开小程序将自动登录工作人员平台。确定绑定吗？',
			success: async (res) => {
				if (res.confirm) {
					try {
						wx.showLoading({ title: '绑定中...' });
						let result = await cloudHelper.callCloudData('work/bind_wx', {});
						wx.hideLoading();
						
						if (result && result.success) {
							wx.showToast({ title: '绑定成功', icon: 'success' });
						}
					} catch (err) {
						wx.hideLoading();
						console.log(err);
						wx.showToast({ title: '绑定失败', icon: 'none' });
					}
				}
			}
		});
	},

	doUnbindWorkWx: async function () {
		wx.showModal({
			title: '解绑微信号',
			content: '解绑后，下次打开小程序将不再自动登录。解绑后需等待7天才能再次绑定。确定解绑吗？',
			success: async (res) => {
				if (res.confirm) {
					try {
						wx.showLoading({ title: '解绑中...' });
						let result = await cloudHelper.callCloudData('work/unbind_wx', {});
						wx.hideLoading();
						
						if (result && result.success) {
							wx.showToast({ title: '解绑成功', icon: 'success' });
						}
					} catch (err) {
						wx.hideLoading();
						console.log(err);
						wx.showToast({ title: err.message || '解绑失败', icon: 'none' });
					}
				}
			}
		});
	},

})