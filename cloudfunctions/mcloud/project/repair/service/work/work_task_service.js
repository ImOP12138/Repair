...
class WorkTaskService extends BaseProjectWorkService {
	...
	async runWorkTask(id, forms) {
		...
		data.TASK_RUN_FORMS = forms;
		data.TASK_RUN_OBJ = dataUtil.dbForms2Obj(forms);
	...
	async overWorkTask(id, forms) {
		...
		let data = {};
		data.TASK_STATUS = TaskModel.STATUS.OVER;
		data.TASK_OVER_FORMS = forms;
		data.TASK_OVER_OBJ = dataUtil.dbForms2Obj(forms);
		data.TASK_OVER_TIME = timeUtil.time();
		await TaskModel.edit(where, data);
	}
