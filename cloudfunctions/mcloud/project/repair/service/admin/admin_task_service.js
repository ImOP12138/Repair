...
class AdminTaskService extends BaseProjectAdminService {
	...
	async apptTaskMember(admin, taskId, memberId) {
		let task = await TaskModel.getOne({ _id: taskId });
		if (!task) this.AppError('报修单不存在');

		let member = await MemberModel.getOne({ _id: memberId });
		...
		let data = {};
		data.TASK_STATUS = TaskModel.STATUS.APPT;
		data.TASK_MEMBER_ID = memberId;
		data.TASK_MEMBER_NAME = member.MEMBER_TITLE;
		data.TASK_MEMBER_PHONE = member.MEMBER_PHONE || '';
		data.TASK_MEMBER_CATE_NAME = member.MEMBER_CATE_NAME;
		data.TASK_MEMBER_CATE_ID = member.MEMBER_CATE_ID;
		data.TASK_MEMBER_TIME = timeUtil.time();
		data.TASK_MEMBER_ADMIN_ID = admin._id;
		data.TASK_MEMBER_ADMIN_NAME = admin.ADMIN_NAME;
	...
	async exportTaskDataExcel(condition, fields) {
		...
		let list = await TaskModel.getAll(where, '*', orderBy, 1000);
		...
		for (let k in list) {
			...
			for (let j in fields) {
				...
				if (field === 'TASK_STATUS') {
					val = TaskModel.STATUS_DESC[val] || val;
				} else if (field === 'TASK_ADD_TIME') {
		...
		return await exportUtil.exportDataExcel(EXPORT_TASK_DATA_KEY, title, list.length, data);
	}
