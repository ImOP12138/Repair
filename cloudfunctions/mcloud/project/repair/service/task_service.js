...
class TaskService extends BaseProjectService {
	...
	async editTask({
		...
		let data = {};
		data[formsName] = forms;
		data[objName] = dataUtil.dbForms2Obj(forms);
	...
	async commentTask(id, forms) {
		...
		let data = {};
		data.TASK_COMMENT_FORMS = forms;
		data.TASK_COMMENT_OBJ = dataUtil.dbForms2Obj(forms);
		data.TASK_COMMENT_TIME = timeUtil.time();
		data.TASK_COMMENT_STATUS = 1;
		await TaskModel.edit(where, data);
	}
